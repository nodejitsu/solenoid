...
var clean = exports.clean = function clean(options, callback) {
  ...
  async.forEachSeries(commands, function (args, next) {
    ...
    child.stdout.on('data', function (d) { winston.info('' + d) });
    child.stderr.on('data', function (d) { winston.info('' + d) });
...
var fetch = exports.fetch = function fetch(options, callback) {
  ...
  function doFetch(callback) {
    ...
    stream.on('error', function (err) {
      winston.error('Error while fetching snapshot: ' + err.message);
    ...
    stream.on('end', function () {
      winston.info('Application snapshot fetched.');
      callback();
    });
  }

  winston.info('Fetching application snapshot...');
  doFetch(callback);
};
...
var unpack = exports.unpack = function unpack(options, callback) {
  ...
  winston.info('Unpacking snapshot...');
...
var readPackageJSON = exports.readPackageJSON = function readPackageJSON(options, callback) {
  winston.info('Reading `package.json`...');
...
var startApp = exports.startApp = function startApp(options, callback) {
  ...
  if (start.length === 1) {
    ...
    else if (start[0] === 'coffee') {
      args.push(path.join(path.dirname(options.enginePath), 'coffee'), start[1]);
    }
  }
