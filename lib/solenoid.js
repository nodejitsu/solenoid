...
exports.start = function (options, callback) {
  ...
  options.decompSnapshotPath = path.join(options.runDir, 'snapshot.tar');
  ...
  async.series([
    ...
    async.apply(decompress, options),
...
var decompress = exports.decompress = function decompress(options, callback) {
  var logger = options.logger,
      finished = false,
      input = fs.createReadStream(options.packedSnapshotPath),
      output = fs.createWriteStream(options.decompSnapshotPath),
      gunzip = zlib.createGunzip();

  input.pipe(gunzip).pipe(output);

  input.on('error', onFinish);
  output.on('error', onFinish);
  gunzip.on('error', onFinish);
...
var unpack = exports.unpack = function unpack(options, callback) {
  ...
  mkdirp(options.tmpPath, function (err) {
    ...
    child = spawn('tar', ['-xf', options.decompSnapshotPath], {
      cwd: snapshotPath
    });
