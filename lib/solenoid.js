...
var startApp = exports.startApp = function startApp(options, callback) {
  var forzaLog = path.join(options.runDir, 'forza.log'),
  ...
  //
  args = ['start', '-o', forzaLog, '--', 'forza', '--start-log', startLog];
    ...
    function awaitStart() {
      async.series([
        ...
        function tailLog(next) {
          ...
          tail.stdout.on('data', function onTail(data) {
            allLog += data;

            try { json = JSON.parse(allLog) }
            catch (ex) { }

            logger.info('Start info: ' + JSON.stringify(json));
            tail.stdout.removeListener(onTail);
            tail.kill();
            next();
          });
        }
