      ...
      async.series([
        ...
        function tailLog(next) {
          ...
          tail.on('close', function (code) {
            logger.info('Tail exiting');
            return typeof err !== 'undefined' || code != 0
              ? retry(err || new Error('tailing start log exited poorly with code ' + code))
              : next()
          });
          tail.stdout.pipe(concat(function (data) {
            logger.info('We got some data!?');
            //
            // If we are actually getting here force it to a string for parsing
            //
