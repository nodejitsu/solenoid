  ...
  fs.exists(options.pidFile, function (exists) {
    ...
    fs.readFile(options.pidFile, 'utf8', function (err, content) {
      ...
      if (Number.isNaN(content)) {
        return callback(new Error('No valid pid in ' + options.pidFile));
      }
...
exports.stop = function (options, callback) {
  fs.readFile(options.pidFile, 'utf8', function (err, content) {
    ...
    try {
      process.kill(content);

      setTimeout(function () {
        try {
          process.kill(content, 'SIGKILL');
        }
        catch (ex_) {
        }
      }, 10000);
    ...
    async.parallel([
      async.apply(rimraf, options.runDir),
      async.apply(fs.unlink, options.pidFile),
      function removeEnv(next) {
        if (!options.envFile) { return next() }
        fs.exists(options.envFile, function (exists) {
          return exists
            ? fs.unlink(options.envFile, next)
            : next();
        });
      }
    ], callback);
...
var unpack = exports.unpack = function unpack(options, callback) {
  ...
  options.tmpPath      = path.join(options.packagePath, '.tmp');
...
var readEnv = exports.readEnv = function readEnv(options, callback) {
  if (!options.app || !options.app.envFile) {
    return callback();
  }

  fs.readFile(options.app.envFile, 'utf8', function (err, data) {
    if (err) {
      return callback();
    }
