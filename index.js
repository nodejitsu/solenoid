...
exports.start = function (options, callback) {
  ...
  return options.old
    ? exports.download(options, runApp)
    : runApp();
};
...
var normalize = function normalize (options) {
  options.packedSnapshotPath = options.packedSnapshotPath || path.join(options.runDir, 'snapshot.tgz');
  options.snapshotPath       = options.snapshotPath || path.join(options.runDir, 'snapshot');
  options.packagePath        = options.packagePath || path.join(options.snapshotPath, 'package');
  options.tmpPath            = options.tmpPath || path.join(options.snapshotPath, '.tmp');
  return options;
};
